...
import os 
from utils_ss import load_data,dynamic_kmeans, static_kmeans,plot_results,elbow_method,create_netCDF,sort_clusters,plot_results1, haversine,relative_angle_cyclone_shoreline
...
def tracking_ss(basin, ii, folder, eps, min_samples, delta, threshold_peak,variable_name,pathout,ncentroids):
    ...
    data_winds0   = load_data(path_winds, 'winds',year0, month0, yearf, monthf,basin)
    data_slp0     = load_data(path_slp, 'slp',year0, month0, yearf, monthf, basin)


    try: 
        data_winds = data_winds0['wind_speed'].max(dim="time", skipna=True)
        data_winds = data_winds.rename({"valid_time": "time"})


        data_slp    = data_slp0['msl'].max(dim="time", skipna=True)
        data_slp    = data_slp.rename({"valid_time": "time"})


    except: 
        data_winds = data_winds0.copy()
        data_slp   = data_slp0.copy()
    ...
    create_netCDF(xarray_z_filtered, xarray_time,x_filtered, y_filtered,track_lat, track_lon, track_wind_speed, track_storm_dir, track_storm_speed, track_slp,track_RMW,SID, path_output,variable_name)
    ...
    try:
        ...
        #plt.plot(wind_at_stations.values)
        # --- Interpolate SLP and Wind to each station location ---
        lons = ds_extended["lon"].values
        lons_era5 = np.where(lons < 0, lons + 360, lons)  # Convert -87 → 273
        lats = ds_extended["station_y_coordinate"].values

        # Interpolate SLP
        slp_at_stations = data_slp.interp(
            longitude=xr.DataArray(lons_era5, dims="stations"),
            latitude=xr.DataArray(lats, dims="stations"))

        # Remove duplicated times and match with ds_extended
        common_times = np.intersect1d(ds_extended.time.values, slp_at_stations.time.values)
        slp_at_stations = slp_at_stations.sel(time=~slp_at_stations.get_index("time").duplicated())
        slp_at_stations = slp_at_stations.sel(time=common_times)

        # Interpolate Winds
        wind_at_stations = data_winds.interp(
            longitude=xr.DataArray(lons_era5, dims="stations"),
            latitude=xr.DataArray(lats, dims="stations"))
        wind_at_stations = wind_at_stations.sel(time=~wind_at_stations.get_index("time").duplicated())
        wind_at_stations = wind_at_stations.sel(time=common_times)

        # --- Extend TC data over the same time dimension ---
        data_TC['time'] = pd.to_datetime(data_TC['ISO_TIME'])
        data_TC = data_TC.set_index('time')
        data_TC_extend = data_TC.reindex(time_extended)

        # --- Assign everything into ds_extended ---
        ...
        ds_extended = ds_extended.assign(
            ...
            # Add interpolated SLP and Winds for all stations
            slp=(("time", "stations"), slp_at_stations.values),
            wind=(("time", "stations"), wind_at_stations.values)
        )

        # Save to NetCDF
        output_file = path_output / f"{SID}_all_time_series.nc"
...
def hydrograph(basin, ii, threshold, folder,variable_name,pathout):
    ...
    data = xr.load_dataset(path_file / f"{SID}_ERA5_ss.nc")

    data = data.assign_coords({"latitude": ("station", data.latitude.values),
                               "longitude": ("station", data.longitude.values)})
    
  
    data_all = xr.load_dataset(path_file / f"{SID}_all_time_series.nc") 
    ...
    for i in range(len(ds_clean.station)):
        ...
        winds_all  = data_all.wind.values[:, i]
        slp_all    = data_all.slp.values[:, i]
        ...
        wind_segment = winds_all[start_idx:end_idx+1]
        slp_segment = slp_all[start_idx:end_idx+1]
        ...
        localwinds_max = wind_segment[np.argmax(surge_segment)]
        localslp_max = slp_segment[np.argmax(surge_segment)]
        ...
        localwinds_dur= np.nanmean(wind_segment)
        localslp_dur= np.nanmean(slp_segment)
        ...
        df_hydrograph.loc[i] = [SID,i,lon.values,lat.values,t0, tf, max_surge, time_max, t_asc, t_des, dur,
                        ...
                        dist_max,rel_dist_max,localwinds_max,localslp_max, relangle_max,
                        wind_dur, dir_dur, rmw_dur, pmin_dur, sspeed_dur,localwinds_dur,localslp_dur]
